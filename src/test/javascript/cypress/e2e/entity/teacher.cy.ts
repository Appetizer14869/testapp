...
describe('Teacher e2e test', () => {
  ...
  const teacherSample = { firstName: 'Tamara', lastName: 'Nader', email: 'Shad_Jenkins55@hotmail.com' };
  ...
  describe('Teacher page', () => {
    ...
    describe('with existing value', () => {
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/teachers?page=0&size=20>; rel="last",<http://localhost/api/teachers?page=0&size=20>; rel="first"',
              },
              body: [teacher],
            },
      ...
      it('last delete button click should delete instance of Teacher', () => {
        cy.intercept('GET', '/api/teachers/*').as('dialogDeleteRequest');
  ...
  describe('new Teacher page', () => {
    ...
    it('should create an instance of Teacher', () => {
      cy.get(`[data-cy="firstName"]`).type('Marilie');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Marilie');

      cy.get(`[data-cy="lastName"]`).type('Sipes');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Sipes');

      cy.get(`[data-cy="email"]`).type('Geraldine75@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Geraldine75@gmail.com');

      cy.get(`[data-cy="phoneNumber"]`).type('eek');
      cy.get(`[data-cy="phoneNumber"]`).should('have.value', 'eek');

      cy.get(`[data-cy="hireDate"]`).type('2025-09-06T22:08');
      cy.get(`[data-cy="hireDate"]`).blur();
      cy.get(`[data-cy="hireDate"]`).should('have.value', '2025-09-06T22:08');

      cy.get(`[data-cy="salary"]`).type('3963');
      cy.get(`[data-cy="salary"]`).should('have.value', '3963');
