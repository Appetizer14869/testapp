...
describe('Department e2e test', () => {
  ...
  const departmentSample = { name: 'whether whose' };
  ...
  describe('new Department page', () => {
    ...
    it('should create an instance of Department', () => {
      cy.get(`[data-cy="name"]`).type('indeed whenever where');
      cy.get(`[data-cy="name"]`).should('have.value', 'indeed whenever where');

      cy.get(`[data-cy="description"]`).type('in');
      cy.get(`[data-cy="description"]`).should('have.value', 'in');
