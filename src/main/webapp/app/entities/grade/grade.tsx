...
import { getEntities, reset } from './grade.reducer';
...
export const Grade = () => {
  ...
  const links = useAppSelector(state => state.grade.links);
  ...
  return (
    <div>
      <h2 id="grade-heading" data-cy="GradeHeading">
        <Translate contentKey="testappApp.grade.home.title">Grades</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="testappApp.grade.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/grade/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="testappApp.grade.home.createLabel">Create new Grade</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={gradeList ? gradeList.length : 0}
          next={handleLoadMore}
          ...
          {gradeList && gradeList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="testappApp.grade.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  ...
                  <th className="hand" onClick={sort('dateAwarded')}>
                    <Translate contentKey="testappApp.grade.dateAwarded">Date Awarded</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('dateAwarded')} />
                  </th>
                  <th>
                    <Translate contentKey="testappApp.grade.student">Student</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="testappApp.grade.subject">Subject</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {gradeList.map((grade, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{grade.score}</td>
                    <td>{grade.remarks}</td>
                    <td>{grade.dateAwarded ? <TextFormat type="date" value={grade.dateAwarded} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{grade.student ? <Link to={`/student/${grade.student.id}`}>{grade.student.id}</Link> : ''}</td>
                    <td>{grade.subject ? <Link to={`/subject/${grade.subject.id}`}>{grade.subject.id}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button
                          onClick={() => (window.location.href = `/grade/${grade.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="testappApp.grade.home.notFound">No Grades found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
