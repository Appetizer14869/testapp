  ...
  return (
    <div>
      <h2 id="student-heading" data-cy="StudentHeading">
        <Translate contentKey="testappApp.student.home.title">Students</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="testappApp.student.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/student/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="testappApp.student.home.createLabel">Create new Student</Translate>
          </Link>
        </div>
      </h2>
        ...
        {studentList && studentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="testappApp.student.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="testappApp.student.classroom">Classroom</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="testappApp.student.parent">Parent</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {studentList.map((student, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {student.enrollmentDate ? <TextFormat type="date" value={student.enrollmentDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{student.classroom ? <Link to={`/class-room/${student.classroom.id}`}>{student.classroom.id}</Link> : ''}</td>
                  <td>{student.parent ? <Link to={`/parent/${student.parent.id}`}>{student.parent.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="testappApp.student.home.notFound">No Students found</Translate>
            </div>
          )
        )}
      </div>
