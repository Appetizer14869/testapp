...
public class CourseRepositoryWithBagRelationshipsImpl implements CourseRepositoryWithBagRelationships {
    ...
    Course fetchSubjects(Course result) {
        return entityManager
            .createQuery("select course from Course course left join fetch course.subjects where course.id = :id", Course.class)
    ...
    List<Course> fetchSubjects(List<Course> courses) {
        ...
        List<Course> result = entityManager
            .createQuery("select course from Course course left join fetch course.subjects where course in :courses", Course.class)
